...
ele_chr = ij_dc.getElementById("ele_chart")
ele_out1 = ij_dc.getElementById("ele_out1")
ele_out2 = ij_dc.getElementById("ele_out2")
ele_out3 = ij_dc.getElementById("ele_out3")
...
def ff_lp_main():
    ...
    """
    if dd_00['url_tgt'] == '':
        ij_wd.console.log(f" tgt none {dd_00['url_tgt']}")
        return
    else:
        ij_wd.console.log(f" tgt select {dd_00['url_tgt']}")
        #return
    ...
    plot = ij_wd.Plotly.newPlot("ele_chart", ij_wd.JSON.parse(graphJSON))
    dt_nw = ii_dt.datetime.now()
    ii_pys.display(f"lp edは{dt_nw:%Y年%m月%d日 %H:%M:%S.%f}です。", target="ele_datenn", append=False)
    window.console.log(f"@ -- st {dd_00['dt_nws']:%Y年%m月%d日 %H:%M:%S.%f} ")
    window.console.log(f"@ -- ed {dt_nw:%Y年%m月%d日 %H:%M:%S.%f} ")
    window.console.log(f"@ -- rep {dd_00['df_4p'].tail(1)} ")
    window.console.log(f"@ -- rep {df_tck.tail(1)} ")
...
def ff_mk_4p(df_tck, aas_smp='1T'):
    ...
    if len(dd_00['df_4p']) != 0:
        ...
        ij_wd.console.log(f"@ -- 4p {dd_00['df_4p'].tail(1)} ")
    else:
        dd_00['df_4p'] = df_4pn
        ij_wd.console.log("@ -- df_4p none")
    ...
    ij_wd.console.log(f"@ -- 4pe {dd_00['df_4pe'].tail(1)} ")
